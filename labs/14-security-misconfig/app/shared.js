...
/**
 * Shared helpers for the security-misconfiguration lab: the HTML shell + panels and
 * a small info/result view. Each stage leaves one distinct misconfiguration in place
 * that leaks the server secret — the security-relevant code shown in the source panel.
 *
...
// The secret each misconfiguration leaks. (The goal text never spells it out.)
const FLAG = 'MISCONFIG{deployment_left_open}';

/** Did a response leak the server secret? */
function leaked(output) {
  return String(output).includes('MISCONFIG{');
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You leaked the secret via a misconfiguration at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** A description card with the endpoints this stage exposes. */
function infoCard(html) {
  return `<div class="card"><h2>🛠️ App status</h2>${html}</div>`;
}

/** A generic result / response panel. */
function outputPanel(title, output) {
  return `<div class="card"><h3>${escapeHtml(title)}</h3><pre>${escapeHtml(typeof output === 'string' ? output : JSON.stringify(output, null, 2))}</pre></div>`;
}

function deniedBanner(msg = '⛔ Not found.') {
  return `<div class="denied">${escapeHtml(msg)}</div>`;
}

module.exports = {
  FLAG, escapeHtml, leaked, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, infoCard, outputPanel, deniedBanner,
};
