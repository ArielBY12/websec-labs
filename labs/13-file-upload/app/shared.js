...
/**
 * Shared helpers for the file-upload lab: the HTML shell + panels and the avatar
 * upload/serve views. Uploads are modeled as {filename, content, mimetype} form
 * fields (no multipart dependency). Each stage owns how it validates the upload and
 * what Content-Type it serves the file with — the security-relevant code.
 *
...
/** Map a filename's extension to the Content-Type a naive server would serve it with. */
function typeByExt(name) {
  const ext = String(name).toLowerCase().match(/\.([a-z0-9]+)$/)?.[1];
  return ({
    html: 'text/html', htm: 'text/html', svg: 'image/svg+xml', js: 'text/javascript',
    png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif',
  })[ext] || 'application/octet-stream';
}

/** Would this served (type, body) run script in a browser? Then the upload is XSS-able. */
function xssable(type, body) {
  return /text\/html|image\/svg/.test(String(type)) && /<script/i.test(String(body));
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! Your upload is served as executable content at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** The upload form (filename + content + claimed MIME, standing in for a real file). */
function uploadForm(ctx, { filename = 'avatar.png', content = '\x89PNG\\r\\n…binary…', mimetype = 'image/png' } = {}) {
  return `<div class="card">
    <h2>🖼️ Upload your avatar</h2>
    <form method="POST" action="${ctx.mount}/upload">
      <label>File name</label><input name="filename" value="${escapeHtml(filename)}">
      <label>File content</label><input name="content" value="${escapeHtml(content)}">
      <label>Claimed Content-Type</label><input name="mimetype" value="${escapeHtml(mimetype)}">
      <button>Upload</button>
    </form>
    <p class="hint">Uploaded files are served back from <code>${ctx.mount}/files/&lt;name&gt;</code>.
      Get one served so a browser executes it (stored XSS).</p>
  </div>`;
}

/** Result panel after an upload, linking to the served file + noting its Content-Type. */
function uploadResult(ctx, storedName, servedType) {
  return `<div class="card"><h3>Stored</h3>
    <p>Served at <a href="${ctx.mount}/files/${encodeURIComponent(storedName)}"><code>${escapeHtml(ctx.mount)}/files/${escapeHtml(storedName)}</code></a>
      with <code>Content-Type: ${escapeHtml(servedType)}</code>.</p></div>`;
}
...
module.exports = {
  escapeHtml, typeByExt, xssable,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, uploadForm, uploadResult, deniedBanner,
};
