...
module.exports = {
  ...
  defense: 'Rejects templates mentioning "global".',
  ...
  createRouter(ctx) {
    ...
    r.post('/render', (req, res) => {
      ...
      if (/global/i.test(tpl))   //! blacklists the word "global" — the runtime is still reachable via this / the constructor chain
        return res.send(shared.stagePage(ctx, { content: shared.templateForm(ctx, tpl), result: shared.deniedBanner('⛔ "global" is not allowed.') }));
