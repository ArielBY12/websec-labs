...
/**
 * Shared helpers for the CORS-misconfiguration lab: the HTML shell + panels and an
 * account-API view. Each stage owns its CORS origin policy — the security-relevant
 * code shown in the source panel.
 *
...
const SECRET = 'CORS{cross_origin_read}';           // returned by /api/account
const TRUSTED = 'https://bank.example';             // the one legitimate front-end origin

/** Could JavaScript running at `origin` read a response whose ACAO header is `acao`? */
function crossOriginReadable(origin, acao) {
  return !!acao && (acao === '*' || acao === origin);
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! An untrusted origin can read the response at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** Info + a form to test whether a given Origin would be allowed to read /api/account. */
function apiCard(ctx, origin = 'https://evil.attacker.com') {
  return `<div class="card">
    <h2>🏦 Account API</h2>
    <p><code>GET ${ctx.mount}/api/account</code> returns your account data and sets CORS
      headers based on the request's <code>Origin</code>. The one legitimate front-end is
      <code>${escapeHtml(TRUSTED)}</code>.</p>
    <form method="GET" action="${ctx.mount}/check">
      <label>Test an Origin</label><input name="origin" value="${escapeHtml(origin)}">
      <button>Check</button>
    </form>
    <p class="hint">Get an <em>untrusted</em> origin allowed to read the credentialed response.</p>
  </div>`;
}

/** Show the CORS decision for a tested origin. */
function decisionPanel(origin, acao) {
  return `<div class="card"><h3>CORS decision</h3>
    <pre>Origin: ${escapeHtml(origin || '(none)')}
Access-Control-Allow-Origin: ${escapeHtml(acao || '(not set — denied)')}</pre></div>`;
}

function deniedBanner(msg = '⛔ Denied.') {
  return `<div class="denied">${escapeHtml(msg)}</div>`;
}

module.exports = {
  SECRET, TRUSTED, crossOriginReadable, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, apiCard, decisionPanel, deniedBanner,
};
