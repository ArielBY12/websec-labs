...
/**
 * Shared helpers for the path-traversal lab: the HTML shell + panels, the on-disk
 * fixtures (a docs directory plus secret files OUTSIDE it), and the viewer view.
 * Each stage owns how it resolves the requested path — the security-relevant code
 * shown in the source panel.
 *
...
// On-disk fixtures in a temp dir: the docs the viewer should serve, plus two secret
// files that live OUTSIDE the docs directory (the traversal targets).
const BASE = fs.mkdtempSync(path.join(os.tmpdir(), 'lab12-'));
const DOCS = path.join(BASE, 'docs');
fs.mkdirSync(DOCS, { recursive: true });
fs.writeFileSync(path.join(DOCS, 'welcome.txt'), 'Welcome to the document viewer.\n');
fs.writeFileSync(path.join(DOCS, 'about.txt'), 'About: our internal knowledge base.\n');
fs.writeFileSync(path.join(BASE, 'secret.txt'), 'PATHTRAVERSAL{files_outside_the_root}\n');   // ../secret.txt
fs.writeFileSync(path.join(BASE, 'docs_secret.txt'), 'PATHTRAVERSAL{sibling_prefix_match}\n'); // ../docs_secret.txt
...
/** Did the viewer read a file it shouldn't — a secret marker or /etc/passwd? */
function escapedDocs(output) {
  return /PATHTRAVERSAL\{|root:.*:0:0:/.test(String(output));
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You read a file outside the docs root at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
module.exports = {
  BASE, DOCS, escapeHtml, escapedDocs,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, viewerForm, outputPanel, deniedBanner,
};
