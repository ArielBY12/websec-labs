...
/**
 * Shared helpers for the insecure-deserialization lab: the HTML shell + panels and a
 * (deliberately unsafe, config-driven) object deserializer. Each stage owns which
 * deserialization features it allows — the security-relevant code in the source panel.
 *
...
// A server secret reachable only by executing code during deserialization (proves RCE).
global.__DESER_FLAG__ = global.__DESER_FLAG__ || 'DESER{unsafe_deserialization_rce}';

const MARKER = '_$$ND_FUNC$$_';   // node-serialize-style function marker

/**
 * A deliberately unsafe deserializer. `opts.functions` evals a function marker (the
 * classic RCE); `opts.reviveTypes` maps a `__type` to a reviver. Both are dangerous.
 */
function unserialize(str, opts = {}) {
  const { functions = false, reviveTypes = null } = opts;
  return JSON.parse(str, function (k, v) {
    if (functions && typeof v === 'string' && v.startsWith(MARKER)) return eval('(' + v.slice(MARKER.length) + ')');
    if (reviveTypes && v && typeof v === 'object' && typeof v.__type === 'string' && reviveTypes[v.__type]) return reviveTypes[v.__type](v);
    return v;
  });
}

/** Did deserialization execute code and expose the secret? */
function deserLeaked(output) {
  return String(output).includes('DESER{');
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You executed code during deserialization at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
// ---------------------------------------------------------------------------
// Session-restore views
// ---------------------------------------------------------------------------

/** The "restore session" form (a serialized preferences token). */
function tokenForm(ctx, token = '{"theme":"dark"}') {
  return `<div class="card">
    <h2>🗄️ Restore preferences</h2>
    <form method="POST" action="${ctx.mount}/restore">
      <label>Serialized preferences token</label><input name="token" value="${escapeHtml(token)}">
      <button>Restore</button>
    </form>
    <p class="hint">The token is deserialized back into an object. Get code to run during
      deserialization and read <code>global.__DESER_FLAG__</code>.</p>
  </div>`;
}

/** Show the deserialized object. */
function resultPanel(obj) {
  let s; try { s = JSON.stringify(obj, (k, v) => (typeof v === 'function' ? '[Function]' : v), 2); } catch { s = String(obj); }
  return `<div class="card"><h3>Restored object</h3><pre>${escapeHtml(String(s))}</pre></div>`;
}
...
module.exports = {
  MARKER, unserialize, deserLeaked, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, tokenForm, resultPanel, deniedBanner,
};
