...
/**
 * Shared helpers for the prototype-pollution lab: the HTML shell + panels and a
 * merge view. Each stage owns how it merges/assigns untrusted input — the
 * security-relevant code shown in the source panel.
 *
...
const SECRET = 'PROTO{prototype_polluted}';   // shown only when Object.prototype is polluted

/** Did the response reveal the pollution secret? */
function polluted(output) {
  return String(output).includes('PROTO{');
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You polluted Object.prototype at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** The merge form (a JSON object to merge into server config). */
function mergeForm(ctx, data = '{"theme":"dark"}') {
  return `<div class="card">
    <h2>⚙️ Update preferences</h2>
    <form method="POST" action="${ctx.mount}/merge">
      <label>Preferences (JSON)</label><input name="data" value="${escapeHtml(data)}">
      <button>Merge</button>
    </form>
    <p class="hint">Your JSON is merged into a settings object. Pollute
      <code>Object.prototype</code> so a freshly-created object inherits your property.</p>
  </div>`;
}

/** Show whether a freshly-created object now carries the polluted gadget. */
function probePanel(gadgetName, isPolluted) {
  return `<div class="card"><h3>Pollution probe</h3>
    <pre>const probe = {};
probe['${escapeHtml(gadgetName)}'] === ${isPolluted ? 'polluted!' : 'undefined'}</pre>
    ${isPolluted ? `<p class="ok">🧬 Object.prototype polluted. Secret: <code>${SECRET}</code></p>` : '<p class="hint">Not polluted yet.</p>'}</div>`;
}
...
module.exports = {
  SECRET, polluted, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, mergeForm, probePanel, deniedBanner,
};
