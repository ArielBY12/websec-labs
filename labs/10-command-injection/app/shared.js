...
/**
 * Shared helpers for the command-injection lab: the HTML shell + panels and a
 * "network tool" view. Each stage owns how it runs the OS command — that's the
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You injected a command at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** The host-lookup form. */
function toolForm(ctx, value = '127.0.0.1') {
  return `<div class="card">
    <h2>🌐 Host lookup</h2>
    <form method="POST" action="${ctx.mount}/lookup">
      <label>Host to look up</label><input name="host" value="${escapeHtml(value)}">
      <button>Look up</button>
    </form>
    <p class="hint">The server runs a shell lookup for the host you enter. Make it run
      a command of your choosing (e.g. <code>id</code>).</p>
  </div>`;
}

/** Render the command's combined output. */
function outputPanel(output) {
  return `<div class="card"><h3>Output</h3><pre>${escapeHtml(output)}</pre></div>`;
}
...
module.exports = {
  escapeHtml, looksInjected, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, toolForm, outputPanel, deniedBanner,
};
