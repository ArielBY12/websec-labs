...
/**
 * Shared helpers for the auth-failures lab: the HTML shell + panels, cookie parsing,
 * and the login/dashboard views. Each stage owns its own in-memory user + session
 * state and implements the (broken) authentication logic — that's the security-
 * relevant code shown in the source panel.
 *
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You took over the admin account at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** The login form, with an optional message and optional "remember me" checkbox. */
function loginForm(ctx, { msg = '', remember = false } = {}) {
...
/** The post-login dashboard. Admins see the flag. */
function dashboard(username, role) {
  const isAdmin = role === 'admin';
  return `<div class="card">
    <h2>👋 Welcome, ${escapeHtml(username)}</h2>
    <p>You are signed in as <strong>${escapeHtml(username)}</strong> (role: <strong>${escapeHtml(role)}</strong>).</p>
    ${isAdmin
      ? `<p class="ok">🔑 Admin console unlocked. Flag: <code>${FLAG}</code></p>`
      : `<p class="hint">Regular users can't see the admin console. Become <code>admin</code>.</p>`}
  </div>`;
}

function deniedBanner(msg = '⛔ Invalid credentials.') {
  return `<div class="denied">${escapeHtml(msg)}</div>`;
}

module.exports = {
  FLAG, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, loginForm, dashboard, deniedBanner,
};
