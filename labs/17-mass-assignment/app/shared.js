...
/**
 * Shared helpers for the mass-assignment lab: the HTML shell + panels and a profile
 * editor view. Each stage owns how it applies the request body to the user object —
 * the security-relevant code shown in the source panel.
 *
...
const SECRET = 'MASSASSIGN{privileged_field_set}';   // shown only once you gain admin

/** Did the response reveal the admin-only secret? */
function gotAdmin(output) {
  return String(output).includes('MASSASSIGN{');
}

/** Recursive object merge (used by a stage whose blacklist only checks top-level keys). */
function deepMerge(target, src) {
  for (const k of Object.keys(src)) {
    if (src[k] && typeof src[k] === 'object' && !Array.isArray(src[k]) && typeof target[k] === 'object' && target[k])
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You set a privileged field via mass assignment at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** The profile update form (a JSON patch in a single field). */
function profileForm(ctx, data = '{"name":"Alice"}') {
  return `<div class="card">
    <h2>👤 Edit profile</h2>
    <form method="POST" action="${ctx.mount}/update">
      <label>Profile patch (JSON)</label><input name="data" value="${escapeHtml(data)}">
      <button>Save</button>
    </form>
    <p class="hint">The patch is merged into your account. You start as a regular user —
      become an admin.</p>
  </div>`;
}

/** Show the current user object, plus the secret if they're now an admin. */
function profilePanel(user, isAdmin) {
  return `<div class="card"><h3>Your account</h3>
    <pre>${escapeHtml(JSON.stringify(user, null, 2))}</pre>
    ${isAdmin ? `<p class="ok">🔑 You are an admin. Secret: <code>${SECRET}</code></p>` : '<p class="hint">role: not admin.</p>'}</div>`;
}
...
module.exports = {
  SECRET, gotAdmin, deepMerge, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, profileForm, profilePanel, deniedBanner,
};
