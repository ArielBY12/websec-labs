...
/**
 * Shared helpers for the cryptographic-failures lab: the HTML shell + panels and a
 * token issue/verify view. Each stage "protects" an auth token with a different
 * broken scheme — the security-relevant code shown in the source panel.
 *
...
const SECRET = 'CRYPTO{forged_admin_token}';   // shown only when a forged admin token verifies

/** Did a response reveal the admin-only secret? */
function forged(output) {
  return String(output).includes('CRYPTO{');
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You forged an admin token at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
// ---------------------------------------------------------------------------
// Token issue / verify views
// ---------------------------------------------------------------------------

/** Show the token issued for alice + a form to verify a (possibly forged) token. */
function tokenCard(ctx, issuedToken) {
  return `<div class="card">
    <h2>🎫 Your session token</h2>
    <p>You are <strong>alice</strong> (role: user). Your token:</p>
    <pre>${escapeHtml(issuedToken)}</pre>
    <form method="GET" action="${ctx.mount}/verify">
      <label>Present a token</label><input name="token" value="${escapeHtml(issuedToken)}">
      <button>Verify</button>
    </form>
    <p class="hint">Forge a token the server accepts as <code>role: admin</code>.</p>
  </div>`;
}

function resultPanel(title, output) {
  return `<div class="card"><h3>${escapeHtml(title)}</h3><pre>${escapeHtml(output)}</pre></div>`;
}

function deniedBanner(msg = '⛔ Invalid token.') {
  return `<div class="denied">${escapeHtml(msg)}</div>`;
}

/** Build the /verify response from the parsed token ({user, role} or null). */
function verifyPage(ctx, parsed, aliceToken) {
  const card = tokenCard(ctx, aliceToken);
  if (!parsed) return stagePage(ctx, { content: card, result: deniedBanner() });
  if (parsed.role === 'admin')
    return stagePage(ctx, { content: card, result: resultPanel('Admin verified', `secret: ${SECRET}`), success: true });
  return stagePage(ctx, { content: card, result: resultPanel('Verified', `Welcome ${parsed.user} (role: ${parsed.role})`) });
}

module.exports = {
  SECRET, forged, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, tokenCard, resultPanel, deniedBanner, verifyPage,
};
