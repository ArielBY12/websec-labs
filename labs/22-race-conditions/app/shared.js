...
/**
 * Shared helpers for the race-conditions lab: the HTML shell + panels and a "claim a
 * one-time bonus" view. Each stage owns how it guards the one-time action — the
 * security-relevant code shown in the source panel. The bug is always an `await`
...
/** Simulate an async I/O gap (DB call, network) — the window a race exploits. */
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! You claimed the one-time bonus more than once at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/** Status card showing how many times the one-time bonus has been granted. */
function statusCard(ctx, granted) {
  return `<div class="card">
    <h2>🎁 One-time signup bonus</h2>
    <p>The $500 bonus may be claimed <strong>once</strong>. Times granted so far:
      <strong>${granted}</strong>.</p>
    <p class="hint"><code>POST ${ctx.mount}/claim</code> claims it; <code>GET ${ctx.mount}/status</code>
      reports the count; <code>POST ${ctx.mount}/reset</code> resets. Fire many
      <code>/claim</code> requests <em>concurrently</em> to get it granted more than once.</p>
  </div>`;
}

function deniedBanner(msg = '⛔ Already claimed.') {
  return `<div class="denied">${escapeHtml(msg)}</div>`;
}

module.exports = {
  delay, escapeHtml, parseCookies,
  page, nav, hintPanel, goalBanner, solvedBanner, sourcePanel, successExplanation,
  recapPanel, stagePage, statusCard, deniedBanner,
};
