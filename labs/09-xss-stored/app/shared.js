...
/**
 * Shared helpers for the stored-XSS lab: the HTML shell + panels and a comment-board
 * view. Each stage owns its own in-memory comment list and implements the (broken)
 * sanitization/rendering — that's the security-relevant code shown in the source panel.
 *
...
/**
 * Does this rendered HTML fragment contain a *real* (unescaped) executable
 * construct — a live tag, an event handler outside a quoted value, or a
 * javascript: href? Escaped output (`&lt;img … onerror=…&gt;`) deliberately does
 * NOT match, since it can't execute. Used to detect a successful stored-XSS.
 */
function looksExecutable(html) {
  return /<script[\s>]|<svg[\s>]|<img\b[^>]*\son\w+\s*=|"\s+on\w+\s*=\s*"|href\s*=\s*"javascript:/i.test(html);
}
...
function solvedBanner(ctx, success) {
  return success && ctx.status !== 'secure'
    ? `<div class="solved">🎉 Solved! Your stored payload executed at Stage ${ctx.stage} — ${escapeHtml(ctx.title)}.</div>`
    : '';
}
...
/**
 * The comment form. `fields` lists the inputs to render (name + label +
 * placeholder), so a stage can add an attribute-context field (author) or a
 * URL field (website) as its vulnerable surface.
 */
function commentForm(ctx, fields = [{ name: 'text', label: 'Comment', placeholder: 'Say something…' }]) {
  const inputs = fields.map((f) =>
    `<label>${escapeHtml(f.label)}</label><input name="${f.name}" placeholder="${escapeHtml(f.placeholder || '')}" value="${escapeHtml(f.value || '')}">`
  ).join('');
  return `<div class="card">
    <h2>💬 Product reviews</h2>
    <form method="POST" action="${ctx.mount}/comment">${inputs}<button>Post review</button></form>
    <p class="hint">Reviews are stored and shown to every visitor. Get a script to run when they view the page.</p>
  </div>`;
}

/** Wrap a rendered list of comment HTML strings in the board. */
function board(commentsHtml) {
  return `<div class="card"><h3>Latest reviews</h3>${commentsHtml || '<p class="hint">No reviews yet.</p>'}</div>`;
}
